...
`openapi.yaml` 1주차 명세를 채워나가는 **Spring Boot 스타터 템플릿**입니다.
CORS, 에러 응답 포맷, 프로젝트 골격은 이미 완성되어 있고, `TodoController.java`와
`FileController.java` 안의 **TODO만 채우면 됩니다.**
...
3. 아무 엔드포인트나 호출해보세요. 아직 구현을 안 했기 때문에 이렇게 나옵니다.
   ...
   ```
4. `TodoController.java`, `FileController.java`를 열어서 TODO 주석을 하나씩
   실제 로직으로 바꾸세요. 채울 때마다 다시 실행해서 curl로 확인하세요.

## 이미 되어 있는 것 / 내가 채워야 하는 것

| 이미 되어 있음 (건드릴 필요 없음) | 내가 채워야 함 |
|---|---|
| `WebConfig.java` — CORS 설정 | `TodoController.java`의 각 메서드 본문 |
| `ApiException.java`, `ErrorResponse.java`, `GlobalExceptionHandler.java` — 공통 에러 포맷 | `FileController.java`의 각 메서드 본문 |
| `Todo.java` — 응답 스키마 | |
| `pom.xml`, `application.properties` — 프로젝트 설정 | |
